        ...
        try:
            with tb.open_file(path + encoder_file[2:] + ".h5") as h5file:
                ...
                for array in h5file.walk_nodes("/"):
                    B.append(array)
                COMP={}
                COMP={'ENC_bias_A'   :np.array([np.zeros(320)],dtype=float).T,
                     'ENC_weights_A' :np.array(B[-3][:],dtype=float),
                     'DEC_bias_A'    :np.array([B[-2][:]],dtype=float).T,
                     'DEC_weights_A' :np.array(B[-1][:],dtype=float)}
                print COMP['ENC_bias_A'].shape
                print COMP['DEC_bias_A'].shape
        except:
            #Wavelet compression
